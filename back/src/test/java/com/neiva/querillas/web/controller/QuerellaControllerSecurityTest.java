...
class QuerellaControllerSecurityTest {
  ...
  private String bodyCrearValido() throws Exception {
    ...
    dto.setNaturaleza(Naturaleza.OFICIO);
    dto.setTemaId(1L);
    dto.setComunaId(1L);
  ...
  // ===== Asignar inspector: DIRECTORA / INSPECTOR -> 200; AUXILIAR -> 403
  @Test @WithMockUser(username="insp", roles={"INSPECTOR"})
  void asignar_ok_inspector() throws Exception {
    var dto = new AsignarInspectorDTO();
    dto.setInspectorId(5L);
    Mockito.when(service.asignarInspector(eq(1L), any(), anyLong())).thenReturn(QuerellaResponse.builder().id(1L).build());

    mvc.perform(put("/api/querellas/1/inspector")
        .contentType(MediaType.APPLICATION_JSON)
        .content(om.writeValueAsString(dto)))
      .andExpect(status().isOk());
  }
